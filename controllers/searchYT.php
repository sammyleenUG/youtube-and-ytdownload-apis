<?php
    ...
    function searchYT($api_key,$max,$q){
        $youtube = file_get_contents(
            'https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults='.$max.'&q='.$q.'&order=date&type=video&videoEmbeddable=true&videoCategoryId=10&key='.$api_key
        );
        ...
        for($x = 0; $x < $max; $x++){
            ...
            $day = substr($youtube_api_response['items'][$x]['snippet']['publishedAt'],0,10);
            $vidId = $youtube_api_response['items'][$x]['id']['videoId'];
            ...
            echo '
                ...
                <tr>
                    ...
                    <td>'.$vid.' &rarr; <span class="font-weight-bold">'.$channel.'</span></td>
